...
// Confirmed transfers only — API-Football has no "rumors" endpoint (rumors
// come from newsSource.js's transfer-category RSS feeds instead).
async function fetchConfirmedTransfers(teamId) {
  ...
  return raw.map((t) => ({
    ...
    type: t.transfers?.[0]?.type, // e.g. 'Free', 'Loan', or a fee
  }));
}

// Minimum 3 characters, same rule as team search.
...
async function fetchMatchStatistics(fixtureId) {
  ...
  return raw.map((teamStats) => ({
    ...
    stats: (teamStats.statistics ?? []).map((s) => ({ type: s.type, value: s.value })),
  }));
}

async function fetchMatchLineups(fixtureId) {
  const raw = await apiGet('/fixtures/lineups', { fixture: fixtureId });
  return raw.map((teamLineup) => ({
    team: teamLineup.team.name,
    formation: teamLineup.formation,
    startXI: (teamLineup.startXI ?? []).map((p) => ({ name: p.player.name, number: p.player.number, position: p.player.pos })),
